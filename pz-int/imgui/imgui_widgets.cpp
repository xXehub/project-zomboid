...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_ScrollbarBg), 0.0f, window_rounding_corners);
    bb.Expand(ImVec2(-ImClamp((float)(int)((bb_width - 2.0f) * 0.5f), 0.0f, 1.0f), -ImClamp((float)(int)((bb_height - 2.0f) * 0.5f), 0.0f, 1.0f)));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	// Render dropdown arrow — instant flip: down=closed, up=open
	{
		const float arrow_size = 5.0f; // fixed pixel size for consistency
		
		// Center the arrow in the right side of the combo box
		ImVec2 center = ImVec2(
			frame_bb.Max.x - style.FramePadding.x - arrow_size - 2.0f,
			frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) * 0.5f
		);
		
		ImU32 arrow_col = GetColorU32(ImGuiCol_Text);
		
		if (popupOpen) {
			// Arrow UP (open)
			ImVec2 a = center + ImVec2(0.0f, -arrow_size * 0.5f);
			ImVec2 b = center + ImVec2(-arrow_size * 0.5f, arrow_size * 0.5f);
			ImVec2 c = center + ImVec2(+arrow_size * 0.5f, arrow_size * 0.5f);
			g.CurrentWindow->DrawList->AddTriangleFilled(a, b, c, arrow_col);
		} else {
			// Arrow DOWN (closed)
			ImVec2 a = center + ImVec2(0.0f, arrow_size * 0.5f);
			ImVec2 b = center + ImVec2(-arrow_size * 0.5f, -arrow_size * 0.5f);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags) {
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
        ...
        if (BeginPopup("picker")) {
            ...
            ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayHSV | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_AlphaBar;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if (flags & ImGuiColorEditFlags_AlphaBar)
		...
		alpha_bar = true; // Default: show alpha bar when alpha is enabled
