...
ImGuiStyle::ImGuiStyle()
{
    ...
    ScrollbarSize = 5.0f;             // Width of the vertical scrollbar (thin, skeet-style)
    ScrollbarRounding = 0.0f;             // No rounding for scrollbar grab (skeet-style)
...
void ImGui::EndGroupBox()
{
	...
	if (window->ScrollbarY) {
		...
		const float arrow_sz = 3.5f; // small triangle size
		ImU32 arrow_col = ImColor(160, 160, 160, 255);

		if (window->Scroll.y > 1.0f) {
			// Fade gradient at top edge
			window->DrawList->AddRectFilledMultiColor(minTop + ImVec2(2, 1), maxTop + ImVec2(1, 4), ImColor(17, 17, 17, 255), ImColor(17, 17, 17, 255), ImColor(17, 17, 17, 0), ImColor(17, 17, 17, 0));

			// Up arrow triangle (centered at top-right)
			ImVec2 center = ImVec2(maxTop.x - 10.0f, minTop.y + 6.0f);
			ImVec2 a = center + ImVec2(0.0f, -arrow_sz * 0.5f);           // tip (top)
			ImVec2 b = center + ImVec2(-arrow_sz * 0.5f, arrow_sz * 0.5f); // base left
			ImVec2 c = center + ImVec2(+arrow_sz * 0.5f, arrow_sz * 0.5f); // base right
			window->DrawList->AddTriangleFilled(a, b, c, arrow_col);
		}
		...
		if ((GetScrollMaxY() - window->Scroll.y) > 1.f) {
			...
			// Down arrow triangle (centered at bottom-right)
			ImVec2 center = ImVec2(maxBottom.x - 10.0f, maxBottom.y + 8.0f);
			ImVec2 a = center + ImVec2(0.0f, arrow_sz * 0.5f);             // tip (bottom)
			ImVec2 b = center + ImVec2(-arrow_sz * 0.5f, -arrow_sz * 0.5f); // base left
			ImVec2 c = center + ImVec2(+arrow_sz * 0.5f, -arrow_sz * 0.5f); // base right
			window->DrawList->AddTriangleFilled(a, b, c, arrow_col);
		}
